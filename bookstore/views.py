...
def create_user_form(request):
    ...
    return render(request, 'administrator/add_user.html', choice)


class ADeleteUser(SuccessMessageMixin, DeleteView):
    model = User
    template_name='administrator/confirm_delete3.html'
...
def create_user(request):
    ...
    if request.method == 'POST':
            ...
            print(userType)
            if userType == "Member":
                a = User(first_name=first_name, last_name=last_name, username=username, email=email, password=password, is_member=True)
                a.save()
                ...
                messages.success(request, 'User was not created')
...
class ACreateChat(LoginRequiredMixin, CreateView):
	form_class = ChatForm
	model = Chat
	template_name = 'administrator/chat_form.html'
...
class AListChat(LoginRequiredMixin, ListView):
	model = Chat
	template_name = 'administrator/chat_list.html'
...
def aabook_form(request):
	return render(request, 'administrator/add_book.html')
...
class ABookListView(LoginRequiredMixin,ListView):
	model = Book
	template_name = 'administrator/book_list.html'
	context_object_name = 'books'
	paginate_by = 3
...
class AManageBook(LoginRequiredMixin,ListView):
	model = Book
	template_name = 'administrator/manage_books.html'
	context_object_name = 'books'
	paginate_by = 3
...
class ADeleteBook(LoginRequiredMixin,DeleteView):
	model = Book
	template_name = 'administrator/confirm_delete2.html'
...
class ADeleteBookk(LoginRequiredMixin,DeleteView):
	model = Book
	template_name = 'administrator/confirm_delete.html'
...
class AViewBook(LoginRequiredMixin,DetailView):
	model = Book
	template_name = 'administrator/book_detail.html'
...
class ADeleteRequest(LoginRequiredMixin,ListView):
	model = DeleteRequest
	template_name = 'administrator/delete_request.html'
	context_object_name = 'feedbacks'
	paginate_by = 3
...
class AFeedback(LoginRequiredMixin,ListView):
	model = Feedback
	template_name = 'administrator/feedback.html'
	context_object_name = 'feedbacks'
	paginate_by = 3
...
def asearch(request):
    ...
    if( len(data) == 0):
        return redirect('administrator')
                ...
                if files:
                    return render(request,'administrator/result.html',{'files':files,'word':word})
                return render(request,'administrator/result.html',{'files':files,'word':word})
